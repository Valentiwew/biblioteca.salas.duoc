...
public class TipoSalaControllerTest {
    ...
    public void testGetAllTipoSalas() throws Exception {
        when(tipoSalaService.listarTodos()).thenReturn(List.of(tipoSala));
    ...
    public void testGetTipoSalaById() throws Exception {
        when(tipoSalaService.obtenerPorId(1)).thenReturn(tipoSala);
    ...
    public void testCreateTipoSala() throws Exception {
        when(tipoSalaService.guardar(any(TipoSala.class))).thenReturn(tipoSala);
    ...
    public void testDeleteTipoSala() throws Exception {
        doNothing().when(tipoSalaService).eliminarPorId(1);

        mockMvc.perform(delete("/api/tipoSalas/1"))
                .andExpect(status().isOk());

        verify(tipoSalaService, times(1)).eliminarPorId(1);
    }
}
