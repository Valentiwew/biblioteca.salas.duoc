...
public class SalaControllerTest {
    ...
    public void testGetAllSalas() throws Exception {
        when(salaService.listarTodos()).thenReturn(List.of(sala));
    ...
    public void testGetSalaById() throws Exception {
        when(salaService.obtenerPorId(1)).thenReturn(sala);
    ...
    public void testCreateSala() throws Exception {
        when(salaService.guardar(any(Sala.class))).thenReturn(sala);
    ...
    public void testDeleteSala() throws Exception {
        doNothing().when(salaService).eliminarPorId(1);

        mockMvc.perform(delete("/api/salas/1"))
                .andExpect(status().isOk());

        verify(salaService, times(1)).eliminarPorId(1);
    }
}
